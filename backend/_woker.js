...
function formatLevel(level) {
  const lv = parseInt(level || 0) || 1;
  return 'LV ' + Math.min(lv, CONFIG.MAX_LEVEL);
}
...
async function setMonitorState(env, roomId, state) {
  const { last_events, ...rest } = state;
  await env.DB.prepare(`
    INSERT OR REPLACE INTO monitor_states
    (room_id, state, last_title, last_cover, last_area, last_parent_area, last_online, last_live_time, last_events, last_check, last_update, version)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).bind(
    roomId,
    state.state,
...
async function addNotifyConfig(env, config) {
  ...
  await env.DB.prepare(`
    INSERT INTO notify_configs (id, name, protocol, api_url, chat_id, receiver_key, message_key, template, extra_params, enabled, created_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).bind(
    id,
    config.name,
...
async function getLogs(env) {
  const { results } = await env.DB.prepare('SELECT time, level, message FROM system_logs ORDER BY time DESC LIMIT ?')
    .bind(CONFIG.MAX_LOG_ENTRIES).all();
  return results;
}

async function addLog(level, message, env) {
  const time = new Date().toISOString();
  await env.DB.prepare('INSERT INTO system_logs (time, level, message) VALUES (?, ?, ?)')
    .bind(time, level, message).run();
  // 保留最多 MAX_LOG_ENTRIES 条
  await env.DB.prepare(`
    DELETE FROM system_logs WHERE id NOT IN (
      SELECT id FROM system_logs ORDER BY time DESC LIMIT ?
    )
...
async function processRoom(roomId, env, options) {
  ...
  try {
    ...
    const prev = await getMonitorState(env, roomId);
...
async function monitorAll(env, options) {
  ...
  await addLog('info', `开始批量检查 ${roomIds.length} 个房间${options.force ? ' (强制刷新)' : ''}`, env);
  const results = [];
...
async function handleRequest(request, env) {
  ...
  if (path === '/api/login' && method === 'POST') {
    ...
    if (username === env.ADMIN_USER && password === env.ADMIN_PASSWORD) {
      ...
      const headers = {
        ...corsHeaders(env),
        'Set-Cookie': `auth=${auth}; HttpOnly; Path=/; Max-Age=86400; Domain=.262832.xyz; SameSite=None; Secure`,
  ...
  if (path === '/api/logout' && method === 'POST') {
    const headers = {
      ...corsHeaders(env),
      'Set-Cookie': 'auth=; HttpOnly; Path=/; Max-Age=0; Domain=.262832.xyz; SameSite=None; Secure',
      'Content-Type': 'application/json'
    };
    return new Response(JSON.stringify({ success: true }), { headers });
  }
  ...
  if (path === '/api/rooms' && method === 'POST') {
    ...
    // 立即同步一次
    try { await processRoom(roomId, env, { force: true }); } catch (e) { /* ignore */ }
  ...
  if (path === '/api/notify-configs' && method === 'DELETE') {
    ...
    try { body = await request.json(); } catch { body = {}; }
    const id = body.id;
    if (!id) return new Response(JSON.stringify({ error: '缺少ID' }), { status: 400, headers: corsHeaders(env) });
  ...
  if (path === '/api/notify-configs/toggle' && method === 'POST') {
    ...
    try {
      ...
      return new Response(JSON.stringify({ error: e.message }), { status: 404, headers: corsHeaders(env) });
    }
  }
  ...
  if (path === '/api/notify-configs/test' && method === 'POST') {
    ...
    try {
      ...
      if (result.success) {
        ...
        return new Response(JSON.stringify({ success: true, message: '测试通知发送成功' }), { headers: corsHeaders(env) });
      } else {
        return new Response(JSON.stringify({ success: false, error: result.error }), { status: 500, headers: corsHeaders(env) });
      }
    } catch (e) {
      return new Response(JSON.stringify({ error: e.message }), { status: 500, headers: corsHeaders(env) });
    }
  }
  ...
  if (path === '/api/monitor' && method === 'POST') {
    ...
    const result = await monitorAll(env, { force });
    return new Response(JSON.stringify(result), { headers: { ...corsHeaders(env), 'Content-Type': 'application/json' } });
  }
  ...
  if (path === '/api/send-live-notify' && method === 'POST') {
    ...
    try {
      ...
      if (success) {
        await addLog('info', `手动发送模拟通知 ${roomId}`, env);
        return new Response(JSON.stringify({ success: true, message: `已发送 ${eventType} 通知` }), { headers: corsHeaders(env) });
      } else {
        return new Response(JSON.stringify({ success: false, error: '通知发送失败，请检查配置' }), { status: 500, headers: corsHeaders(env) });
      }
    } catch (e) {
      return new Response(JSON.stringify({ error: e.message }), { status: 500, headers: corsHeaders(env) });
    }
