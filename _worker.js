...
async function removeRoom(env, roomId) {
    ...
    rooms = rooms.filter(function(id) {
        return String(id) !== String(roomId);
    });
    await setRoomList(env, rooms);
...
async function processRoom(roomId, env) {
    ...
    for (var j = 0; j < events.length; j++) {
        ...
        await sendNotification(text, env, {
            event: evt.type,
            room_id: roomId,
            ...evt.data
        });
...
export default {
    async fetch(request, env) {
        ...
        if (path === '/add-notify' && method === 'POST') {
            ...
            if (!name || !api_url) return new Response('缺少必要字段', {
                status: 400
            });
        ...
        if (path === '/test-notify' && method === 'POST') {
            ...
            try {
                ...
                if (!isLive) return new Response(JSON.stringify({
                    success: false,
                    message: '当前未开播，无法测试'
                }), {
                    status: 400
        ...
        if (path === '/add-room' && method === 'POST') {
            var form = await request.formData();
            var roomId = String(form.get('room_id') || '').trim();
            if (!roomId) {
            ...
            try {
                ...
                await addLog('info', '添加后自动检测完成 ' + roomId);
            } catch (e) {
                await addLog('error', '添加检测失败 ' + e.message);
            }
