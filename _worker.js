...
function removeRoom(env, roomId) {
    return getRoomList(env).then(function(rooms) {
        rooms = rooms.filter(function(id) {
            return id !== roomId;
        });
        return setRoomList(env, rooms);
    });
...
function sendNotification(text, env) {
    return getNotifyConfigs(env).then(function(configs) {
        var enabled = configs.filter(function(c) {
...
function isAuthenticated(request, env) {
    ...
    var authCookie = cookie.split(';').filter(function(c) {
        return c.trim().startsWith('auth=');
    })[0];
...
export default {
    fetch: function(request, env) {
        ...
        if (path === '/clear-logs') {
            if (!isAuthenticated(request, env)) return Promise.resolve(new Response('Unauthorized', {
                status: 401
            }));
        ...
        if (path === '/add-notify' && request.method === 'POST') {
            ...
            return request.formData().then(function(form) {
                ...
                if (!name || !chat_id) return new Response('缺少必要字段', {
                    status: 400
                });
                if (protocol === 'telegram' && !api_url) {
                    ...
                    if (token) {
                        ...
                        return new Response('缺少 Bot Token', {
                            status: 400
                        });
                    }
                }
                if (!api_url) return new Response('缺少 API 地址', {
                    status: 400
                });
                var receiver_key = 'chat_id',
                    message_key = 'text';
                if (protocol === 'onebot_private') {
                    receiver_key = 'user_id';
                    message_key = 'message';
                } else if (protocol === 'onebot_group') {
                    receiver_key = 'group_id';
                    message_key = 'message';
                }
                return addNotifyConfig(env, {
                    name: name,
                    protocol: protocol,
                    api_url: api_url,
                    chat_id: chat_id,
        ...
        if (path === '/delete-notify' && request.method === 'POST') {
            ...
            return request.formData().then(function(form) {
                ...
                if (!id) return new Response('缺少ID', {
                    status: 400
                });
        ...
        if (path === '/test-notify' && request.method === 'POST') {
            ...
            return request.formData().then(function(form) {
                ...
                return getNotifyConfigs(env).then(function(configs) {
                    ...
                    if (!config) return new Response('配置不存在', {
                        status: 404
                    });
                    return getRoomList(env).then(function(roomIds) {
                        if (roomIds.length === 0) {
                ...
                return request.formData().then(function(form) {
                    ...
                    if (username === env.ADMIN_USER && password === env.ADMIN_PASSWORD) {
                        ...
                        return new Response(null, {
                            ...
                            headers: {
                                'Location': redirect,
                                'Set-Cookie': 'auth=' + auth + '; HttpOnly; Path=/; Max-Age=86400'
                            }
                        });
                    } else {
                        return new Response(renderLoginPage(redirect), {
                            headers: {
                                'Content-Type': 'text/html'
                            }
                        });
        ...
        if (path === '/admin' || path === '/add-room' || path === '/remove-room') {
            if (!isAuthenticated(request, env)) {
                return new Response(renderLoginPage('/admin'), {
                    headers: {
                        'Content-Type': 'text/html'
                    },
                    status: 401
        ...
        if (path === '/add-room' && request.method === 'POST') {
            return request.formData().then(function(form) {
                ...
                if (!roomId) return new Response('缺少房间号', {
                    status: 400
                });
        ...
        if (path === '/check') {
            ...
            if (!roomId) return new Response(JSON.stringify({
                error: '缺少 room_id'
            }), {
                status: 400
            });
        ...
        return new Response('B站直播监控 Worker 已运行。访问 /admin 管理。', {
            headers: {
    ...
    scheduled: function(event, env) {
        ...
        return monitorAll(env).then(function(result) {
            if (result && result.error) {
                ...
                return addLog('error', errMsg).catch(function(e) {
                    console.error('[CRON] 记录错误日志失败:', e.message);
                });
            } else {
                var sucMsg = '定时任务完成，共检查 ' + (result ? result.length : 0) + ' 个房间';
                console.log('[CRON] ' + sucMsg);
                return addLog('info', sucMsg).catch(function(e) {
                    console.error('[CRON] 记录成功日志失败:', e.message);
                });
            }
        }).catch(function(err) {
            var errMsg = '定时任务异常: ' + err.message;
            console.error('[CRON] ' + errMsg);
            addLog('error', errMsg).catch(function(e) {
                console.error('[CRON] 记录异常日志失败:', e.message);
            });
            return Promise.resolve();
        });
    }
